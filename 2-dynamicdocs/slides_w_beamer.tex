...
\begin{quote}
Is it something like \(\frac{1}{365} \times N =\) 0.112?
\end{quote}
...
1 - Simulate 10,000 rooms with \(n = 41\) random birthdays, and store
the results in matrix where each row represents a room.\\
2 - For each room (row) compute the number of unique birthdays.\\
3 - Compute the average number of times a room has 41 unique birthdays,
...
\item
  And the simulated probability is of 0.7288
\end{itemize}
