...
\begin{quote}
There are 37 in this room.
\end{quote}

\begin{quote}
Is it something like \(\frac{1}{365} \times 37=\) 0.101?
\end{quote}
...
1 - Simulate 10,000 rooms with \(n = 37\) random birthdays, and store
the results in matrix where each row represents a room.\\
2 - For each room (row) compute the number of unique birthdays.\\
3 - Compute the average number of times a room has 37 unique birthdays,
...
\item
  And the simulated probability is of 0.7336
\end{itemize}
